$(function(){
    ...
    $("#dg").datagrid({
        ...
        url:'/systemdictionarydetail/page',
        method:"POST",
        toolbar:'#toolbar',
        frozenColumns:[[{field:'aaaa',checkbox:true}]],
        columns:[[
            {field:'id',title:'id',width:100,hidden:true},
            {field:'name',title:'数据字典明细',width:100},
            {field:'types',title:'所属类型',width:100,formatter:format}
        ]]
    });
    //翻页功能
...
window.methods = {
    ...
    save:function () {
        ...
        if($("#systemdictionarydetailid").val()) {
            url = "/systemdictionarydetail/update";
    ...
    add:function () {
        $("#ffff").form("reset");
        $("#systemdictionarydetailid").val("");
        window.methods.loadDictionaryTypes();
    ...
    edit:function () {
        ...
        $.getJSON("/systemdictionarydetail/findOne",{id:selectedRows[0].id},function(data){
    ...
    remove:function () {
        $.messager.confirm('确认','您确认想要删除这些数据吗？',function(r){
            if (r){
                ...
                //发送请求，传递ids参数，进行删除
                $.postJSON("/systemdictionarydetail/delete",{ids:ids.join(",")},function(data){
                    $.messager.alert('信息',data.msg,"info");
                    if(data.status == 200){
                        //刷新表格数据
                        $("#dg").datagrid('load',{pageNo:1,pageSize:10})
    ...
    loadDictionaryTypes:function () {
        $("#typesId").combobox({
            valueField:'id',
            textField:'name',
            url:'/systemdictionarytype/findAllDictionaryTypes',
            editable:false,
            panelHeight:'auto',
            value:'-1'
        });
