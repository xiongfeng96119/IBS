$(function(){
    ...
    $("#dg").datagrid({
        ...
        columns:[[
            ...
            {field:'descs',title:'descs',width:100},
            {field:'productId',title:'productId',width:100},
            {field:'billId',title:'billId',width:100},
        ]]
    });
    //翻页功能
...
window.methods = {
    ...
    save:function () {
        ...
        var url = "/stockincomebillitem/save";
        if($("#stockincomebillitemid").val()) {
            url = "/stockincomebillitem/update";
    ...
    edit:function () {
        ...
        $.getJSON("/stockincomebillitem/findOne",{id:selectedRows[0].id},function(data){
    ...
    remove:function () {
        $.messager.confirm('确认','您确认想要删除这些数据吗？',function(r){
            if (r){
                ...
                //发送请求，传递ids参数，进行删除
                $.postJSON("/stockincomebillitem/delete",{ids:ids.join(",")},function(data){
                    $.messager.alert('信息',data.msg,"info");
                    if(data.status == 200){
                        //刷新表格数据
                        $("#dg").datagrid('load',{pageNo:1,pageSize:10})
