$(function(){
    ...
    $("#dg").datagrid({
        ...
        columns:[[
            ...
            {field:'sn',title:'资源权限',width:200},
            {field:'menu',title:'关联的菜单',width:100,formatter: format},
...
window.methods = {
    ...
    edit:function () {
        ...
        $.getJSON("/permission/findOne",{id:selectedRows[0].id},function(data){
            //表单回填[要求json对象的属性名称要与表单输入框的name属性值一致]
            $("#ffff").form("load", data);
            window.methods.loadMenus();
            //关联的菜单要单独回填一下
            if (data.menu) $("#menuId").combobox("setValue", data.menu.id);
            //显示模态框
            $('#dd').dialog('open');
        });
    },
    ...
    remove:function () {
        $.messager.confirm('确认','您确认想要删除这些数据吗？',function(r){
            if (r){
                ...
                //发送请求，传递ids参数，进行删除
                $.postJSON("/permission/delete",{ids:ids.join(",")},function(data){
                    $.messager.alert('信息',data.msg,"info");
                    if(data.status == 200){
                        //刷新表格数据
                        $("#dg").datagrid('load',{pageNo:1,pageSize:10})
    ...
    loadMenus:function () {
        $("#menuId").combobox({
            valueField:'id',
            textField:'name',
            url:'/menu/findAll?loadParent=false',
            editable:false,
            value:'-1'
        });
    }
