$(function(){
    ...
    $("#dg").datagrid({
        ...
        columns:[[
            ...
            {field:'name',title:'数据字典类型',width:100},
...
window.methods = {
    ...
    save:function () {
        ...
        if($("#systemdictionarytypeid").val()) {
            url = "/systemdictionarytype/update";
    ...
    edit:function () {
        ...
        $.getJSON("/systemdictionarytype/findOne",{id:selectedRows[0].id},function(data){
    ...
    remove:function () {
        $.messager.confirm('确认','您确认想要删除这些数据吗？',function(r){
            if (r){
                ...
                //发送请求，传递ids参数，进行删除
                $.postJSON("/systemdictionarytype/delete",{ids:ids.join(",")},function(data){
                    $.messager.alert('信息',data.msg,"info");
                    if(data.status == 200){
                        //刷新表格数据
                        $("#dg").datagrid('load',{pageNo:1,pageSize:10})
