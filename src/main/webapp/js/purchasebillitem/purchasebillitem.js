...
window.methods = {
    ...
    remove:function () {
        $.messager.confirm('确认','想要删除吗？',function(r){
            if (r){
                ...
                //发送请求进行删除
                $.postJSON("/purchasebillitem/delete",{ids:ids.join(",")},function(data){
    ...
    edit:function () {
        ...
        $("#menu").form("reset");
        $.getJSON("/purchasebillitem/findOne",{id:selectedRows[0].id},function(data){
            $("#menu").form("load", data);
            //打开模态框
            $('#dd').dialog('open');
        });
    ...
    add:function () {
        $("#menu").form("reset");
        $("#purchasebillitemid").val("");
        $('#dd').dialog('open');  // open a window
    },
    save:function () {
        var params = $("#menu").toJson();
        var url = "/purchasebillitem/save";
        if($("#purchasebillitemid").val()) {
            url = "/purchasebillitem/update";
