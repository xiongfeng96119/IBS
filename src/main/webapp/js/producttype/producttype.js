$(function(){
    ...
    $("#dg").datagrid({
        ...
        columns:[[
            ...
            {field:'parent',title:'父级类型名称',width:100,formatter:format},
        ]]
    });
    //翻页功能
...
window.methods = {
    ...
    add:function () {
        $("#ffff").form("reset");
        $("#producttypeid").val("");
        window.methods.loadParentTypes();
    ...
    edit:function () {
        ...
        $.getJSON("/producttype/findOne",{id:selectedRows[0].id},function(data){
            ...
            $("#ffff").form("load", data);
            //关联的父级商品类型要单独回填一下
            if (data.parent) $("#parentId").combobox("setValue", data.parent.id);
            //显示模态框
            $('#dd').dialog('open');
        });
    },
    ...
    loadParentTypes:function () {
        $("#parentId").combobox({
            valueField:'id',
            textField:'name',
            url:'/producttype/findAll?loadParent=true',
            editable:false,
            value:'-1'
        });
    }
