$(function(){
    ...
    $("#dg").datagrid({
        ...
        columns:[[
            ...
            {field:'maxcapacity',title:'maxcapacity',width:100},
            {field:'currentcapacity',title:'currentcapacity',width:100},
            {field:'totalamount',title:'totalamount',width:100},
        ]]
    });
    //翻页功能
...
window.methods = {
    ...
    edit:function () {
        ...
        $.getJSON("/depot/findOne",{id:selectedRows[0].id},function(data){
    ...
    remove:function () {
        $.messager.confirm('确认','您确认想要删除这些数据吗？',function(r){
            if (r){
                ...
                //发送请求，传递ids参数，进行删除
                $.postJSON("/depot/delete",{ids:ids.join(",")},function(data){
                    $.messager.alert('信息',data.msg,"info");
                    if(data.status == 200){
                        //刷新表格数据
                        $("#dg").datagrid('load',{pageNo:1,pageSize:10})
