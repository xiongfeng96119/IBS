$(function(){
    ...
    $("#dg").datagrid({
        ...
        columns:[[
            ...
            {field:'incomedate',title:'incomedate',width:100},
            {field:'warning',title:'warning',width:100},
            {field:'topnum',title:'topnum',width:100},
            {field:'bottomnum',title:'bottomnum',width:100},
            {field:'productId',title:'productId',width:100},
            {field:'depotId',title:'depotId',width:100},
        ]]
    });
    //翻页功能
...
window.methods = {
    ...
    save:function () {
        ...
        if($("#productstockid").val()) {
            url = "/productstock/update";
    ...
    edit:function () {
        ...
        $.getJSON("/productstock/findOne",{id:selectedRows[0].id},function(data){
    ...
    remove:function () {
        $.messager.confirm('确认','您确认想要删除这些数据吗？',function(r){
            if (r){
                ...
                //发送请求，传递ids参数，进行删除
                $.postJSON("/productstock/delete",{ids:ids.join(",")},function(data){
                    $.messager.alert('信息',data.msg,"info");
                    if(data.status == 200){
                        //刷新表格数据
                        $("#dg").datagrid('load',{pageNo:1,pageSize:10})
